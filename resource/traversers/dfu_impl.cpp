...
bool dfu_impl_t::is_enough (const share_tally_map_t &tallies, unsigned int multiplier)
{
    return std::all_of (tallies.begin (), tallies.end (), [multiplier] (const auto &kv) {
        return kv.second.satisfies (multiplier);
    });
}
                                     ...
                                     unsigned int multiplier)
{
    ...
    for (auto &kv : outedges) {
        ...
        // Once a resource type has sufficient shares, no need to find more
        auto t_iter = tallies.find ((*m_graph)[tgt].type);
        if (t_iter != tallies.end () && t_iter->second.satisfies (multiplier))
            continue;
        ...
        if (rc == 0) {
            ...
            rc2 = 0;
            if (is_enough (tallies, multiplier))
                break;
        }
    }
    return rc2;
